    ...
    <div className="flex gap-2 justify-center">
      {otp.map((digit, index) => (
        <input
          ...
          onPaste={handlePaste}
          disabled={disabled}
          className={`${size === 'compact' ? 'w-10 h-10' : 'w-12 h-12'} text-center text-lg font-bold border rounded-lg outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed backdrop-blur-sm ${inputClasses}`}
        />
      ))}
    </div>
  );
