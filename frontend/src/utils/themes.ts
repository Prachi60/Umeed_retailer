export interface Theme {
  primary: string[];
  secondary: string[];
  textColor: string;
  accentColor: string;
  bannerText: string;
  saleText: string;
  headerTextColor: string;
}

export const themes: Record<string, Theme> = {
  all: {
    primary: ['#FFF3E0', '#FFB74D', '#F57C00', '#FF9800'],
    secondary: ['#FFF3E0', '#FFE0B2', '#FFCC80', '#FFB74D'],
    textColor: '#000000',
    accentColor: '#F57C00',
    bannerText: 'HOUSEFULL',
    saleText: 'SALE',
    headerTextColor: '#000000',
  },
  wedding: {
    primary: ['rgb(252, 165, 165)', 'rgb(253, 182, 182)', 'rgb(254, 202, 202)', 'rgb(255, 228, 228)'],
    secondary: ['rgb(255, 228, 228)', 'rgb(254, 202, 202)', 'rgb(253, 182, 182)'],
    textColor: '#7f1d1d',
    accentColor: '#991b1b',
    bannerText: 'WEDDING',
    saleText: 'SALE',
    headerTextColor: '#7f1d1d',
  },
  winter: {
    primary: ['rgb(186, 230, 253)', 'rgb(191, 234, 255)', 'rgb(207, 250, 254)', 'rgb(224, 242, 254)'],
    secondary: ['rgb(224, 242, 254)', 'rgb(207, 250, 254)', 'rgb(191, 234, 255)'],
    textColor: '#0c4a6e',
    accentColor: '#075985',
    bannerText: 'WINTER',
    saleText: 'SALE',
    headerTextColor: '#0c4a6e',
  },
  electronics: {
    primary: ['rgb(255, 126, 0)', 'rgb(255, 145, 36)', 'rgb(255, 163, 71)', 'rgb(255, 182, 107)'],
    secondary: ['rgb(255, 230, 195)', 'rgb(255, 210, 170)', 'rgb(255, 190, 140)'],
    textColor: '#713f12',
    accentColor: '#854d0e',
    bannerText: 'ELECTRONICS',
    saleText: 'SALE',
    headerTextColor: '#713f12',
  },
  beauty: {
    primary: ['rgb(251, 207, 232)', 'rgb(252, 218, 238)', 'rgb(253, 224, 239)', 'rgb(254, 240, 246)'],
    secondary: ['rgb(254, 240, 246)', 'rgb(253, 224, 239)', 'rgb(252, 218, 238)'],
    textColor: '#831843',
    accentColor: '#9f1239',
    bannerText: 'BEAUTY',
    saleText: 'SALE',
    headerTextColor: '#831843',
  },
  grocery: {
    primary: ['rgb(187, 247, 208)', 'rgb(209, 250, 229)', 'rgb(220, 252, 231)', 'rgb(236, 253, 245)'],
    secondary: ['rgb(236, 253, 245)', 'rgb(220, 252, 231)', 'rgb(209, 250, 229)'],
    textColor: '#14532d',
    accentColor: '#166534',
    bannerText: 'GROCERY',
    saleText: 'SALE',
    headerTextColor: '#14532d',
  },
  fashion: {
    primary: ['rgb(196, 181, 253)', 'rgb(205, 192, 255)', 'rgb(221, 214, 254)', 'rgb(237, 233, 254)'],
    secondary: ['rgb(237, 233, 254)', 'rgb(221, 214, 254)', 'rgb(205, 192, 255)'],
    textColor: '#4c1d95',
    accentColor: '#5b21b6',
    bannerText: 'FASHION',
    saleText: 'SALE',
    headerTextColor: '#4c1d95',
  },
  sports: {
    primary: ['rgb(147, 197, 253)', 'rgb(165, 208, 255)', 'rgb(191, 219, 254)', 'rgb(219, 234, 254)'],
    secondary: ['rgb(219, 234, 254)', 'rgb(191, 219, 254)', 'rgb(165, 208, 255)'],
    textColor: '#1e3a8a',
    accentColor: '#1e40af',
    bannerText: 'SPORTS',
    saleText: 'SALE',
    headerTextColor: '#1e3a8a',
  },
  orange: {
    primary: ['rgb(251, 146, 60)', 'rgb(253, 186, 116)', 'rgb(254, 215, 170)', 'rgb(255, 237, 213)'],
    secondary: ['rgb(255, 237, 213)', 'rgb(254, 215, 170)', 'rgb(253, 186, 116)'],
    textColor: '#9a3412',
    accentColor: '#c2410c',
    bannerText: 'AUTUMN',
    saleText: 'SALE',
    headerTextColor: '#7c2d12',
  },
  violet: {
    primary: ['rgb(167, 139, 250)', 'rgb(196, 181, 253)', 'rgb(221, 214, 254)', 'rgb(237, 233, 254)'],
    secondary: ['rgb(237, 233, 254)', 'rgb(221, 214, 254)', 'rgb(196, 181, 253)'],
    textColor: '#4c1d95',
    accentColor: '#5b21b6',
    bannerText: 'VIOLET',
    saleText: 'SALE',
    headerTextColor: '#2e1065',
  },
  teal: {
    primary: ['rgb(45, 212, 191)', 'rgb(94, 234, 212)', 'rgb(153, 246, 228)', 'rgb(204, 251, 241)'],
    secondary: ['rgb(204, 251, 241)', 'rgb(153, 246, 228)', 'rgb(94, 234, 212)'],
    textColor: '#115e59',
    accentColor: '#0f766e',
    bannerText: 'TEAL',
    saleText: 'SALE',
    headerTextColor: '#134e4a',
  },
  dark: {
    primary: ['rgb(75, 85, 99)', 'rgb(107, 114, 128)', 'rgb(156, 163, 175)', 'rgb(209, 213, 219)'],
    secondary: ['rgb(209, 213, 219)', 'rgb(156, 163, 175)', 'rgb(107, 114, 128)'],
    textColor: '#ffffff',
    accentColor: '#1f2937',
    bannerText: 'DARK',
    saleText: 'SALE',
    headerTextColor: '#000000',
  },
  hotpink: {
    primary: ['rgb(244, 114, 182)', 'rgb(249, 168, 212)', 'rgb(251, 207, 232)', 'rgb(253, 224, 239)'],
    secondary: ['rgb(253, 224, 239)', 'rgb(251, 207, 232)', 'rgb(249, 168, 212)'],
    textColor: '#831843',
    accentColor: '#9d174d',
    bannerText: 'PINK',
    saleText: 'SALE',
    headerTextColor: '#831843',
  },
  gold: {
    primary: ['#F57C00', '#FB8C00', '#FF9800', '#FFA726'],
    secondary: ['#FFCC80', '#FFE0B2', '#FFF3E0'],
    textColor: '#9a3412',
    accentColor: '#a16207',
    bannerText: 'GOLD',
    saleText: 'SALE',
    headerTextColor: '#713f12',
  },
  skyblue: {
    primary: ['rgb(125, 211, 252)', 'rgb(186, 230, 253)', 'rgb(224, 242, 254)'],
    secondary: ['rgb(224, 242, 254)', 'rgb(186, 230, 253)'],
    textColor: '#0c4a6e',
    accentColor: '#0284c7',
    bannerText: 'SKY',
    saleText: 'SALE',
    headerTextColor: '#0c4a6e',
  },
  lime: {
    primary: ['rgb(190, 242, 100)', 'rgb(217, 249, 157)', 'rgb(236, 252, 203)'],
    secondary: ['rgb(236, 252, 203)', 'rgb(217, 249, 157)'],
    textColor: '#365314',
    accentColor: '#65a30d',
    bannerText: 'FRESH',
    saleText: 'SALE',
    headerTextColor: '#365314',
  },
  rose: {
    primary: ['rgb(251, 113, 133)', 'rgb(253, 164, 175)', 'rgb(255, 228, 230)'],
    secondary: ['rgb(255, 228, 230)', 'rgb(253, 164, 175)'],
    textColor: '#881337',
    accentColor: '#e11d48',
    bannerText: 'ROSE',
    saleText: 'SALE',
    headerTextColor: '#881337',
  },
  indigo: {
    primary: ['rgb(129, 140, 248)', 'rgb(165, 180, 252)', 'rgb(224, 231, 255)'],
    secondary: ['rgb(224, 231, 255)', 'rgb(165, 180, 252)'],
    textColor: '#1e1b4b',
    accentColor: '#4f46e5',
    bannerText: 'INDIGO',
    saleText: 'SALE',
    headerTextColor: '#1e1b4b',
  },
  silver: {
    primary: ['rgb(209, 213, 219)', 'rgb(229, 231, 235)', 'rgb(243, 244, 246)'],
    secondary: ['rgb(243, 244, 246)', 'rgb(229, 231, 235)'],
    textColor: '#374151',
    accentColor: '#4b5563',
    bannerText: 'SILVER',
    saleText: 'SALE',
    headerTextColor: '#374151',
  },
  charcoal: {
    primary: ['rgb(55, 65, 81)', 'rgb(75, 85, 99)', 'rgb(107, 114, 128)'],
    secondary: ['rgb(107, 114, 128)', 'rgb(75, 85, 99)'],
    textColor: '#ffffff',
    accentColor: '#1f2937',
    bannerText: 'CHARCOAL',
    saleText: 'SALE',
    headerTextColor: '#ffffff',
  },
  forest: {
    primary: ['rgb(34, 197, 94)', 'rgb(74, 222, 128)', 'rgb(187, 247, 208)'],
    secondary: ['rgb(187, 247, 208)', 'rgb(74, 222, 128)'],
    textColor: '#064e3b',
    accentColor: '#16a34a',
    bannerText: 'FOREST',
    saleText: 'SALE',
    headerTextColor: '#064e3b',
  },
  cheeryred: {
    primary: ['rgb(239, 68, 68)', 'rgb(248, 113, 113)', 'rgb(254, 202, 202)'],
    secondary: ['rgb(254, 202, 202)', 'rgb(248, 113, 113)'],
    textColor: '#7f1d1d',
    accentColor: '#b91c1c',
    bannerText: 'HOT',
    saleText: 'SALE',
    headerTextColor: '#7f1d1d',
  },
};

export const getTheme = (tabId: string): Theme => {
  return themes[tabId] || themes.all;
};
